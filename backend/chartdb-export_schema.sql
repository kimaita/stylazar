...
WITH fk_info AS (
    SELECT array_to_string(
            array_agg(
                CONCAT(
                    ...
                    '"',
                    ',"table":"',
                    replace(table_name::text, '"', ''),
                    '"',
                    ',"column":"',
                    replace(fk_column::text, '"', ''),
                    '"',
                    ',"foreign_key_name":"',
                    foreign_key_name,
                    '"',
                    ',"reference_schema":"',
                    COALESCE(reference_schema, 'public'),
                    '"',
                    ',"reference_table":"',
                    reference_table,
                    '"',
    ...
    FROM (
            SELECT connamespace::regnamespace::text AS schema_name,
                ...
                CASE
                    WHEN strpos(conrelid::regclass::text, '.') > 0 THEN split_part(conrelid::regclass::text, '.', 2)
                    ELSE conrelid::regclass::text
                END AS table_name,
                (
                    regexp_matches(
                        pg_get_constraintdef(oid),
                        '(?i)FOREIGN KEY \("?(\w+)"?\) REFERENCES (?:"?(\w+)"?\.)?"?(\w+)"?\("?(\w+)"?\)',
                        'g'
                    )
                ) [1] AS fk_column,
                (
...
indexes_cols AS (
    SELECT tnsp.nspname AS schema_name,
        ...
        (
            CASE
                WHEN i.indisunique = TRUE THEN 'true'
                ELSE 'false'
            END
        ) AS is_unique,
        ...
        END AS direction,
        CASE
            WHEN indpred IS NOT NULL THEN 'true'
            ELSE 'false'
        END AS is_partial_index
    FROM pg_index AS i
    ...
    SELECT array_to_string(
            array_agg(
                CONCAT(
                    ...
                    COALESCE(cols.character_maximum_length::text, 'null'),
                    '","precision":',
                    CASE
                        WHEN cols.data_type = 'numeric'
                        OR cols.data_type = 'decimal' THEN CONCAT(
                            '{"precision":',
                            COALESCE(cols.numeric_precision::text, 'null'),
                            ',"scale":',
                            COALESCE(cols.numeric_scale::text, 'null'),
                            '}'
                        )
                        ELSE 'null'
                    END,
                    ...
                    '","comment":"',
                    COALESCE(
                        replace(replace(dsc.description, '"', '\"'), '\x', '\\x'),
                        ''
                    ),
                    '"}'
                )
    ...
    SELECT array_to_string(
            array_agg(
                CONCAT(
                    ...
                    '","view_definition":"',
                    encode(
                        convert_to(REPLACE(definition, '"', '\"'), 'UTF8'),
                        'base64'
                    ),
                    '"}'
                )
...
SELECT CONCAT(
        '{    "fk_info": [',
        COALESCE(fk_metadata, ''),
        '], "pk_info": [',
        COALESCE(pk_metadata, ''),
        '], "columns": [',
        COALESCE(cols_metadata, ''),
        '], "indexes": [',
        COALESCE(indexes_metadata, ''),
        '], "tables":[',
        COALESCE(tbls_metadata, ''),
        '], "views":[',
        COALESCE(views_metadata, ''),
        '], "database_name": "',
        CURRENT_DATABASE(),
        '',
        '", "version": "',
